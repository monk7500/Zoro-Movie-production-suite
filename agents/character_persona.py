...
def run(input_slices: Dict[str, Any], bible_version: str, llm_provider) -> Dict[str, bytes]:
    ...
    if not characters:
        empty = {"characters": {}}
        return {"character_personas.json": json.dumps(empty, indent=2).encode("utf-8")}
    ...
    # 4. Add metadata
    output_json = json.dumps(personas, indent=2, ensure_ascii=False)
    personas["_meta"] = {
    ...
    return {"character_personas.json": output_json.encode("utf-8")}
